...
import torch.nn as nn
import torch.nn.functional as F
from app.grid_sample import grid_sample
from torch.autograd import Variable
from app.tps_grid_gen import TPSGridGen
...
def define_Unet(input_nc, gpu_ids=[0]):
    netG = Unet(input_nc)
    netG.cuda(gpu_ids[0])
    netG.apply(weights_init)
    return netG
...
def define_UnetMask(input_nc, gpu_ids=[0]):
    netG = UnetMask(input_nc,output_nc=4)
    netG.cuda(gpu_ids[0])
    netG.apply(weights_init)
    return netG

def define_Refine(input_nc, output_nc, gpu_ids=[0]):
...
def define_D(input_nc, ndf, n_layers_D, norm='instance', use_sigmoid=False, num_D=1, getIntermFeat=False, gpu_ids=[0]):
    norm_layer = get_norm_layer(norm_type=norm)
...
def define_VAE(input_nc, gpu_ids=[0]):
...
def define_B(input_nc, output_nc, ngf, n_downsample_global=3, n_blocks_global=3, norm='instance', gpu_ids=[0]):
    norm_layer = get_norm_layer(norm_type=norm)
...
def define_partial_enc(input_nc, gpu_ids=[0]):
...
def define_conv_enc(input_nc, gpu_ids=[0]):
...
def define_AttG(output_nc, gpu_ids=[0]):
